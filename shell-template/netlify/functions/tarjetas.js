/**
 * Proxy seguro → Google Apps Script (API Tarjeta Pallet).
 *
 * - API_TOKEN y APPS_SCRIPT_URL (o TARJETAS_SCRIPT_URL) SOLO en env de Netlify.
 * - El navegador llama /api/tarjetas; esta función inyecta el token en el servidor.
 * - Panel shell: solo lectura. crearTarjeta / guardarTarjeta están bloqueadas aquí.
 * - CORS: mismo sitio + localhost/127.0.0.1 (Live Server de pruebas).
 *
 * Env (Netlify → Environment variables):
 *   APPS_SCRIPT_URL o TARJETAS_SCRIPT_URL = URL …/exec del Web App de Tarjetas
 *   API_TOKEN = mismo valor que Apps Script → Propiedades → API_TOKEN (mismo que Permisos)
 */
...
const ALLOWED_POST = new Set([
  ...
  "reporteTarjetas",
  "consultarTarjetas"
  // crearTarjeta / guardarTarjeta: NO en el proxy público del panel
]);
...
function scriptConfig() {
  const scriptUrl = String(
    process.env.TARJETAS_SCRIPT_URL ||
      process.env.APPS_SCRIPT_URL ||
      process.env.TARJETAS_API_URL ||
      ""
  ).trim();
  const token = String(process.env.API_TOKEN || process.env.TARJETAS_API_TOKEN || "").trim();
...
exports.handler = async (event) => {
  ...
  if (!scriptUrl) {
    return jsonResponse(event, 500, {
      ...
      api: "tarjetas",
      code: "CONFIG",
      message: "Falta APPS_SCRIPT_URL (o TARJETAS_SCRIPT_URL) en variables de entorno de Netlify"
    });
  }
  if (!token) {
    return jsonResponse(event, 500, {
